...
def main():
    ...
    response = client.post(
        "/api/v1/organizers",
        json={"name": "Test Organizer", "email": test_email, "organization": "Vibe Coders"},
    )
    ...
    response = client.get("/api/v1/hackathons", headers={"X-API-Key": api_key})
    ...
    response = client.post(
        ...
        json={
            "submissions": [
                ...
                {"team_name": "Team Beta", "repo_url": "https://github.com/fastapi/fastapi"},
            ]
        },
    )
    ...
    print("\nNext steps:")
    print("  - Verify data in DynamoDB")
    print("  - Test cost estimation endpoint")
    print("  - Test analysis trigger (placeholder)")
    print()
