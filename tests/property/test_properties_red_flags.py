...
def team_analysis_with_red_flags_strategy(draw: Any) -> TeamAnalysisResult:
    ...
    if contributor_count == 1:
        ...
        raw_values = [draw(st.floats(min_value=0.1, max_value=100.0)) for _ in range(contributor_count)]
    ...
    for contributor in contributors:
        scorecard = IndividualScorecard(
            ...
            files_touched=[f"file_{i}.py" for i in range(draw(st.integers(min_value=0, max_value=20)))],
...
def test_property_33_red_flag_has_all_required_fields(
    red_flag: RedFlag
) -> None:
...
@given(red_flag=red_flag_strategy())
@settings(max_examples=100, deadline=None)
def test_property_33_red_flag_severity_is_valid(
    red_flag: RedFlag
) -> None:
...
@given(red_flag=red_flag_strategy())
@settings(max_examples=100, deadline=None)
def test_property_33_red_flag_evidence_contains_specifics(
    red_flag: RedFlag
) -> None:
...
@given(red_flag=red_flag_strategy())
@settings(max_examples=100, deadline=None)
def test_property_33_red_flag_impact_explains_why_it_matters(
    red_flag: RedFlag
) -> None:
...
@given(red_flag=red_flag_strategy())
@settings(max_examples=100, deadline=None)
def test_property_33_red_flag_hiring_impact_assessment(
    red_flag: RedFlag
) -> None:
...
@given(red_flag=red_flag_strategy())
@settings(max_examples=100, deadline=None)
def test_property_33_red_flag_recommended_action_is_actionable(
    red_flag: RedFlag
) -> None:
...
def test_property_33_red_flag_type_determines_severity(
    flag_type: str
) -> None:
...
def test_property_34_critical_red_flags_trigger_disqualification_recommendation(
    ...
    critical_flags = [
        flag for flag in team_analysis.red_flags
...
def test_property_34_only_critical_flags_trigger_disqualification(
    ...
    if critical_flag_count > 0:
        ...
        assert should_disqualify, \
            f"Should disqualify with {critical_flag_count} critical flags"
...
def test_property_34_specific_critical_flags_disqualify(
    ...
    if len(critical_flags) > 0:
        should_disqualify = True
        assert should_disqualify, \
            f"Should disqualify with critical flags: {critical_flags}"
    else:
        should_not_disqualify = True
...
@settings(max_examples=100, deadline=None)
def test_property_35_no_branches_triggers_code_review_flag(
    ...
    if branch_data["all_commits_to_main"] and not branch_data["has_branches"] and not branch_data["has_pull_requests"]:
        should_flag = True
        expected_flag_type = "no_code_review"
        expected_severity = RedFlagSeverity.MEDIUM
    ...
    if branch_data["has_branches"] or branch_data["has_pull_requests"]:
        ...
        assert should_not_flag, \
            "Should not flag 'no_code_review' when branches or PRs exist"
...
@given(
    all_commits_to_main=st.booleans(),
    has_branches=st.booleans()
)
@settings(max_examples=100, deadline=None)
def test_property_35_main_only_commits_with_branches_is_acceptable(
    all_commits_to_main: bool,
    has_branches: bool
) -> None:
    ...
    if has_branches:
        has_code_review_culture = True
        assert has_code_review_culture, \
            "Should have code review culture when branches exist"
    
    # Property: No branches + all commits to main = no code review
    if not has_branches and all_commits_to_main:
        should_flag = True
        assert should_flag, \
            "Should flag no code review when no branches and all commits to main"


@given(
    has_branches=st.booleans(),
    has_pull_requests=st.booleans(),
    all_commits_to_main=st.booleans()
)
@settings(max_examples=100, deadline=None)
...
@given(team_analysis=team_analysis_with_red_flags_strategy())
@settings(max_examples=50, deadline=None)
def test_property_red_flag_detection_completeness(
    team_analysis: TeamAnalysisResult
) -> None:
    """Integration property: Complete red flag detection.
...
@settings(max_examples=50, deadline=None)
def test_property_red_flag_severity_distribution(
    red_flag_count: int,
    critical_count: int
) -> None:
    ...
    if red_flag_count > 0:
        ...
        assert 0.0 <= critical_ratio <= 1.0, \
            "Critical ratio should be between 0 and 1"
