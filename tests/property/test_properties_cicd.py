...
from src.models.test_execution import TestFramework
...
========================= {passed} passed"""
    ...
    output += (
        f" in {draw(st.floats(min_value=0.1, max_value=60.0)):.2f}s =========================\n"
    )
...
def go_test_output_strategy(draw: Any) -> str:
    ...
    for i in range(passed):
        ...
        output += (
            f"--- PASS: TestFunction{i} ({draw(st.floats(min_value=0.001, max_value=1.0)):.3f}s)\n"
        )

    for i in range(failed):
        output += f"=== RUN   TestFailure{i}\n"
        output += (
            f"--- FAIL: TestFailure{i} ({draw(st.floats(min_value=0.001, max_value=1.0)):.3f}s)\n"
        )
    ...
    output += (
        f"ok      github.com/test/repo    {draw(st.floats(min_value=0.1, max_value=60.0)):.3f}s\n"
    )
...
def test_property_8_fetch_up_to_5_recent_runs(
    run_count: int, fetch_limit: int, data: st.DataObject
) -> None:
...
def test_property_8_handle_fewer_than_5_runs(run_count: int, data: st.DataObject) -> None:
    ...
    with patch.object(analyzer, "_fetch_workflow_logs") as mock_fetch:
        ...
        assert len(result) == run_count, f"Should fetch all {run_count} runs, got {len(result)}"
...
def test_property_10_detect_all_job_types(
    has_lint: bool, has_test: bool, has_build: bool, has_deploy: bool
) -> None:
...
def test_property_11_sophistication_increases_with_features(
    job_count: int, has_caching: bool, has_matrix: bool
) -> None:
...
@settings(max_examples=50, deadline=None)
def test_property_12_exponential_backoff_retry(retry_count: int, backoff_base: float) -> None:
...
def test_property_13_analysis_completes_within_15_seconds(
    workflow_count: int, run_count_per_workflow: int
) -> None:
...
@given(has_workflows=st.booleans(), has_test_output=st.booleans(), has_linter_output=st.booleans())
@settings(max_examples=50, deadline=None)
def test_property_cicd_pipeline_completeness(
    has_workflows: bool, has_test_output: bool, has_linter_output: bool
) -> None:
    """Integration property: Complete CI/CD analysis pipeline.
