...
def test_calculate_workload_distribution_single_contributor(
    ...
    base_repo_data.commit_history = [
        create_commit("Alice", base_time + timedelta(hours=i)) for i in range(10)
    ]
...
def test_detect_ghost_contributor(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
    """Test detection of ghost contributor (0 commits)."""
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [
        create_commit("Alice", base_time + timedelta(hours=i)) for i in range(5)
    ]
...
def test_detect_minimal_contribution(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
...
def test_detect_pair_programming_pattern(
    ...
    pair_patterns = [
        p for p in result.collaboration_patterns if p.pattern_type == "pair_programming"
    ]
...
def test_detect_code_review_pattern(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
...
def test_detect_merge_commits_pattern(
    ...
    for i in range(5):
        commits.append(
            create_commit("Alice", base_time + timedelta(hours=i), f"Merge pull request #{i}")
        )
...
def test_detect_role_backend(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
    """Test detection of backend role from file patterns."""
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [create_commit("Alice", base_time)]
    base_repo_data.source_files = [
...
def test_detect_role_frontend(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
...
def test_detect_expertise_database(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
    """Test detection of database expertise."""
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [create_commit("Alice", base_time, "Add database migration")]
    base_repo_data.source_files = [
...
def test_detect_expertise_security(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
...
def test_detect_expertise_testing(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
    """Test detection of testing expertise."""
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [create_commit("Alice", base_time, "Add unit tests for API")]
    base_repo_data.source_files = [
...
def test_calculate_commit_message_quality_mixed(
    ...
    base_repo_data.commit_history = [
        ...
        create_commit(
            "Alice", base_time + timedelta(hours=2), "Add database migration for users table"
        ),
...
def test_calculate_team_grade_poor(team_analyzer: TeamAnalyzer, base_repo_data: RepoData) -> None:
...
def test_analyze_work_style_frequent_commits(
    ...
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [
        create_commit("Alice", base_time + timedelta(hours=i)) for i in range(25)
    ]
...
def test_analyze_work_style_infrequent_commits(
    ...
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [
        create_commit("Alice", base_time + timedelta(hours=i)) for i in range(3)
    ]
...
def test_generate_hiring_signals_junior(
    ...
    base_time = datetime(2024, 1, 1, 10, 0, tzinfo=UTC)
    base_repo_data.commit_history = [
        create_commit("Alice", base_time + timedelta(hours=i), "fix", insertions=50, deletions=5)
        for i in range(5)
    ]
    base_repo_data.source_files = [
        SourceFile(path="src/main.py", content="", lines=100, language="Python"),
